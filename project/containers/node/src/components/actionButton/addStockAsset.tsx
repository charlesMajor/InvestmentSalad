...
import { APIResult, AssetGet, AssetType, StockCreate, StockGet } from "@/lib/services/returnTypes";

interface AddStockAssetProps {
  setOpen: (open: boolean) => void;
  modifyView: boolean;
  assetId?: string;
}

export function AddStockAsset({ setOpen, modifyView, assetId }: AddStockAssetProps) {
  ...
  const selectedAsset: StockGet = useSelector(selectAssetById(assetId ? assetId : ""));
  ...
  let formSchema = z.object({
    ...
    symbol: z
      ...
      .refine((value) => isSymbolStockInYahooFinance(value), {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const updatedDefaultValues: StockCreate = {
      buyDate: buyDate,
    ...
    if (modifyView) {
      const updateValues: StockGet = {
        ...updatedDefaultValues,
        portfolioId: values.portfolioId,
        id: selectedAsset.id,
      };

      await updateAsset(updateValues as AssetGet, AssetType.STOCK).then(
