package ca.csfoy.investmentSaladApi.api.widgets.watch_list;
...
public class WatchListWidgetRessourceTest {
	@Autowired
	...
	private final WatchListWidgetDto ANY_WATCH_LIST_WIDGET_DTO = new WatchListWidgetDto("Any name", 0, 0, 0, 0,
			WidgetTypes.NET_WORTH, List.of(ANY_TAG_ID), List.of(ANY_SYMBOL));
	...
	void canCreateNetWorthWidgetAndReturn201Created() throws Exception {
		// Arrange
		Mockito.when(controller.createWatchListWidget(ANY_WATCH_LIST_WIDGET_DTO, ANY_DASHBOARD_ID))
				.thenReturn(ANY_WATCH_LIST_WIDGET_FULL_DTO);
		// Act
		mockMvc.perform(MockMvcRequestBuilders.post(PATH_TO_TEST + "/" + ANY_DASHBOARD_ID + "/watchList")
				.contentType(CONTENT_TYPE).content(objectMapper.writeValueAsString(ANY_WATCH_LIST_WIDGET_DTO)))
				.andExpect(MockMvcResultMatchers.status().isCreated());
		// Assert
		Mockito.verify(controller).createWatchListWidget(ANY_WATCH_LIST_WIDGET_DTO, ANY_DASHBOARD_ID);
	...
	void canUpdateNetWorthWidgetAndReturn204NoContent() throws Exception {
		// Arrange
		Mockito.doNothing().when(controller).updateWatchListWidget(ANY_WATCH_LIST_WIDGET_FULL_DTO, ANY_DASHBOARD_ID,
				ANY_ID);
		// Act
		mockMvc.perform(MockMvcRequestBuilders.put(PATH_TO_TEST + "/" + ANY_DASHBOARD_ID + "/watchList/" + ANY_ID)
				.contentType(CONTENT_TYPE).content(objectMapper.writeValueAsString(ANY_WATCH_LIST_WIDGET_FULL_DTO)))
				.andExpect(MockMvcResultMatchers.status().isNoContent());
		// Assert
		Mockito.verify(controller).updateWatchListWidget(ANY_WATCH_LIST_WIDGET_FULL_DTO, ANY_DASHBOARD_ID, ANY_ID);
		Mockito.verifyNoInteractions(converter);
	}
